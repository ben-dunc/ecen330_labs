...
const static double firCoefficients[FIR_FILTER_TAP_COUNT] = {
    4.3579622275120871e-04, 
    2.7155425450406482e-04, 
    6.3039002645024923e-05, 
    -1.9349227837935689e-04, 
    -4.9526428865281100e-04, 
    -8.2651441681321251e-04, 
    -1.1538970332472542e-03, 
    -1.4254746936265958e-03, 
    -1.5744703111426992e-03, 
    -1.5281041447445794e-03, 
    -1.2208092333090721e-03, 
    -6.1008312441272067e-04, 
    3.0761698758505478e-04, 
    1.4840192333212630e-03, 
    2.8123077568332011e-03, 
    4.1290616416555957e-03, 
    5.2263464670258821e-03, 
    5.8739882867061607e-03, 
    5.8504032099208122e-03, 
    4.9787419333799827e-03, 
    3.1637974805960073e-03, 
    4.2435139609134468e-04, 
    -3.0844289197247119e-03, 
    -7.0632027332701809e-03, 
    -1.1078458037608581e-02, 
    -1.4591395057493116e-02, 
    -1.7004337345765962e-02, 
    -1.7720830774014491e-02, 
    -1.6213409845727576e-02, 
    -1.2091458677988312e-02, 
    -5.1609257765542604e-03, 
    4.5319860006883374e-03, 
    1.6679627700682660e-02, 
    3.0718365411587241e-02, 
    4.5861875593064989e-02, 
    6.1160185621895735e-02, 
    7.5579213982547147e-02, 
    8.8092930943210607e-02, 
    9.7778502396672379e-02, 
    1.0390414346016497e-01, 
...
const static double iirACoefficientConstants[FILTER_FREQUENCY_COUNT][IIR_A_COEFFICIENT_COUNT] = {
    ...
    {-4.6377947119071443e+00, 1.3502215749461573e+01, -2.6155952405269751e+01, 3.8589668330738334e+01, -4.3038990303252618e+01, 3.7812927599537112e+01, -2.5113598088113768e+01, 1.2703182701888078e+01, -4.2755083391143458e+00, 9.0332828533800102e-01},
    {-3.0591317915750929e+00, 8.6417489609637510e+00, -1.4278790253808840e+01, 2.1302268283304301e+01, -2.2193853972079225e+01, 2.0873499791105438e+01, -1.3709764520609394e+01, 8.1303553577931709e+00, -2.8201643879900526e+00, 9.0332828533800114e-01},
    {-1.4071749185996758e+00, 5.6904141470697542e+00, -5.7374718273676324e+00, 1.1958028362868898e+01, -8.5435280598354613e+00, 1.1717345583835954e+01, -5.5088290876998611e+00, 5.3536787286077576e+00, -1.2972519209655577e+00, 9.0332828533799836e-01},
    {8.2010906117760429e-01, 5.1673756579268675e+00, 3.2580350909221010e+00, 1.0392903763919218e+01, 4.8101776408669252e+00, 1.0183724507092546e+01, 3.1282000712126901e+00, 4.8615933365572221e+00, 7.5604535083145363e-01, 9.0332828533800569e-01},
...
const static double iirBCoefficientConstants[FILTER_FREQUENCY_COUNT][IIR_B_COEFFICIENT_COUNT] = {
    ...
    {9.0928631323898375e-10, 0.0000000000000000e+00, -4.5464315661949193e-09, 0.0000000000000000e+00, 9.0928631323898386e-09, 0.0000000000000000e+00, -9.0928631323898386e-09, 0.0000000000000000e+00, 4.5464315661949193e-09, 0.0000000000000000e+00, -9.0928631323898375e-10},
    {9.0928660767801113e-10, 0.0000000000000000e+00, -4.5464330383900558e-09, 0.0000000000000000e+00, 9.0928660767801115e-09, 0.0000000000000000e+00, -9.0928660767801115e-09, 0.0000000000000000e+00, 4.5464330383900558e-09, 0.0000000000000000e+00, -9.0928660767801113e-10},
    {9.0928686475841070e-10, 0.0000000000000000e+00, -4.5464343237920538e-09, 0.0000000000000000e+00, 9.0928686475841076e-09, 0.0000000000000000e+00, -9.0928686475841076e-09, 0.0000000000000000e+00, 4.5464343237920538e-09, 0.0000000000000000e+00, -9.0928686475841070e-10},
    {9.0928655942373765e-10, 0.0000000000000000e+00, -4.5464327971186880e-09, 0.0000000000000000e+00, 9.0928655942373761e-09, 0.0000000000000000e+00, -9.0928655942373761e-09, 0.0000000000000000e+00, 4.5464327971186880e-09, 0.0000000000000000e+00, -9.0928655942373765e-10},
...
double filter_iirFilter(uint16_t filterNumber) {
    ...
    for (uint32_t i =0; i < IIR_B_COEFFICIENT_COUNT; i++) {
        z_b += queue_readElementAt(&yQueue, IIR_B_COEFFICIENT_COUNT-1-i) * iirBCoefficientConstants[filterNumber][i];
    }
    // Sum up all of the zQueue elements multiplied by the iir a-coefficients.
    for (uint32_t i =0; i < IIR_A_COEFFICIENT_COUNT; i++) {
        z_a += queue_readElementAt(&zQueues[filterNumber], IIR_A_COEFFICIENT_COUNT-1-i) * iirACoefficientConstants[filterNumber][i];
    }
...
double filter_computePower(uint16_t filterNumber, bool forceComputeFromScratch, bool debugPrint) {
    ...
    if (forceComputeFromScratch) {
        // Compute the power.
        for (uint32_t j = 0; j < OUTPUT_QUEUE_SIZE; j++) {
            newPower += queue_readElementAt(&outputQueues[filterNumber], j) * queue_readElementAt(&outputQueues[filterNumber], j);
        }
    } 
    // If the power does not need to be computed from scratch, use the previous and newest power values to compute the new power.
    else {
        newPower = currentPowerValue[filterNumber] - (oldestValue[filterNumber] * oldestValue[filterNumber]) + (newestValue * newestValue);
    }
...
void filter_getNormalizedPowerValues(double normalizedArray[], uint16_t *indexOfMaxValue) {
    ...
    // Iterate through the current power array and find the index of the maximum power value.
    ...
    // Iterate through the power array and normalize each value by dividing it by the maximum power value.
    for (uint16_t i=0; i < FILTER_IIR_FILTER_COUNT; i++) {
